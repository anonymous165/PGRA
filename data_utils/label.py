...
import hashlib
from data_utils.graph import Graph
...
class Label:
    ...
    def split(self, nd_train=0, n_val=500, n_test=1000, label_type=None, seed=config.seed):
        ...
        if nd_train <= 0:
            ...
            if 0 < n_val < 1:
                n_val = int(n_val * len(left_samples))
            if 0 < n_test < 1:
                n_test = int(n_test * len(left_samples))
            ...
            for _data in data:
                ...
                if label is not None:
                    if label not in self.directed_edge_types:
                        sample = tuple([*sorted(sample[:2]), *sample[2:]])
